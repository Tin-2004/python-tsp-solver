...
def main():
    ...
    if algorithm_type == "GA":
        ...
        print(f"\\n开始遗传算法优化...")
        # 更新打印信息以反映精英比例
        print(f"参数设置: 种群大小={population_size}, 精英保留比例={elite_rate}, 变异率={mutation_rate}, 交叉率={crossover_rate}, 迭代代数={generations}")
        ...
        algorithm_name = "遗传算法"
        # 更新绘图标题参数以反映精英比例
        plot_title_params = f"种群: {population_size}, 精英比例: {elite_rate}, 变异率: {mutation_rate}, 交叉率: {crossover_rate}, 代数: {generations}"
    ...
    if best_route_indices:
        ...
        print(f"算法运行时间: {execution_time:.4f} 秒") 
        if average_distances_progress:
            print(f"最后一代/迭代的平均距离: {average_distances_progress[-1]:.4f}")
        # 绘制结果，传入平均距离数据
